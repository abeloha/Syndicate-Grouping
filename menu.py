...
class Ui(QtWidgets.QDialog):
    ...
    switch_course = QtCore.pyqtSignal(int) #course 1 = jr, 2 = snr
    ...
    def __init__(self):
        ...
        self.jrBtn = self.findChild(QtWidgets.QPushButton, 'jrBtn')
        self.jrBtn.clicked.connect(self.jrBtnPressed)

        self.snrBtn = self.findChild(QtWidgets.QPushButton, 'snrBtn')
        self.snrBtn.clicked.connect(self.snrBtnPressed)
    ...
    def get_current_session(self):
        ...
        if conn is not None:
            with conn:
                ...
                self.label_session_name.setText("Current Year: " + str(self.current_session_name))
        else:
            db.conn_error_handle()

    def jrBtnPressed(self):
        if (not self.check_current_session()):
            return

        self.switch_course.emit(1)

    def snrBtnPressed(self):
        if (not self.check_current_session()):
            return
            
        self.switch_course.emit(2)
    ...
    def check_current_session(self):
        ...
        self.show_message('Set current year', 'Please set current year first.\n\nClick CHANGE YEAR to do so.')
        return 0
